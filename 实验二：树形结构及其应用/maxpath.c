...
int maxget=0x80000000;

int maxfind(TreeNode* root)
{
    if(!root)return 0;

    int leftmax=fang_max(maxfind(root->left),0);
    int rightmax=fang_max(maxfind(root->right),0);
    int thismax=root->val+leftmax+rightmax;
    maxget=fang_max(maxget,thismax);
    return root->val+fang_max(leftmax,rightmax);
}
