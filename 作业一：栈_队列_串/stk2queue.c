...
int GetHead(Queue Q, DataType *e)
{
    ...
    if(StackEmpty(*(Q.stack_out)))
    {
        ...
        //InitStack(Q.stack_in);
        if(StackEmpty(*(Q.stack_out)))return 0;
        else
        {
...
{
    //todo
    if(Q->stack_in->top==MaxSize-1&&Q->stack_out->top==MaxSize-1)return 0;
    else
    {
        ...
        {
            //if(Q->stack_out->top + Q->stack_in->top + 2 > MaxSize)return 0;
            if(StackEmpty(*(Q->stack_out)))
            {
                while (!StackEmpty(*(Q->stack_in))) {
                    DataType x;
...
void QueueToArray(Queue Q, DataType *seq)
{
    //todo
    int i = 0;
    for (; i <= Q.stack_out->top; i++)
    {
        seq[i] = Q.stack_out->data[Q.stack_out->top-i];
    }
    for (; i <= Q.stack_in->top+Q.stack_out->top+1; i++)
    {
        seq[i] = Q.stack_in->data[i-Q.stack_out->top-1];
    }
}
